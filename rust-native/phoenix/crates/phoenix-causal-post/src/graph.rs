use phoenix_semantic_v2::{
    CausalChainId, CausalChainRecord, CausalClaimStatus, CausalEdgeAddition,
    CounterfactualReason, CounterfactualReviewRecord,
};
...
pub fn build_graph_stats(edges: &[CausalEdgeAddition]) -> CausalGraphStats {
    ...
    for edge in edges {
        ...
        *incoming_by_target.entry(node_key(&edge.target)).or_default() += 1;
...
pub fn build_chain_records(edges: &[CausalEdgeAddition], created_at: i64) -> Vec<CausalChainRecord> {
    let mut adjacency = FxHashMap::<String, Vec<&CausalEdgeAddition>>::default();
    for edge in edges {
        if !matches!(edge.status, CausalClaimStatus::Active | CausalClaimStatus::Supported) {
            continue;
        }
        adjacency.entry(node_key(&edge.source)).or_default().push(edge);
    }

    let mut chains = Vec::new();
    let mut seen = FxHashSet::default();
    for first in edges {
        if !matches!(first.status, CausalClaimStatus::Active | CausalClaimStatus::Supported) {
            continue;
        }
        ...
        for second in next_edges {
            if first.document_id != second.document_id || first.relation_kind != second.relation_kind {
                continue;
            }
            ...
            let temporal_consistency_millis = temporal_consistency(&first.effective_interval, &second.effective_interval);
...
pub fn build_counterfactual_reviews(
    ...
    for edge in edges {
        by_target
            .entry(format!("{}:{:?}", node_key(&edge.target), edge.relation_kind))
            .or_default()
            .push(edge);
        by_source
            .entry(format!("{}:{:?}", node_key(&edge.source), edge.relation_kind))
            .or_default()
            .push(edge);
    }
    ...
    for (_target, mut competing) in by_target {
        ...
        let support_path = chains
            .iter()
            .find(|chain| chain.edge_ids.iter().any(|edge_id| edge_id == &best.edge_id))
    ...
    for (_source, outgoing) in by_source {
        if outgoing.len() == 1 {
            ...
            let only_support_path = chains
                .iter()
                .find(|chain| chain.edge_ids.iter().any(|edge_id| edge_id == &edge.edge_id))
                .map(|chain| chain.chain_id.clone());
            if let Some(path_id) = only_support_path {
                reviews.push(CounterfactualReviewRecord {
                    review_id: phoenix_semantic_v2::CausalReviewId(format!("review:brittle:{}", edge.edge_id.0)),
                    case_id: edge.case_id.clone(),
                    focal_edge_id: edge.edge_id.clone(),
                    document_id: edge.document_id.clone(),
                    source: edge.source.clone(),
...
fn edge_signature(
    ...
    format!("{}:{}:{relation_kind:?}", node_key(source), node_key(target))
}
