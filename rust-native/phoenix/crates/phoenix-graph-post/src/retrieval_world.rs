...
use crate::retrieval_common::{
    build_region_from_snapshot, kernel_from_snapshot, retrieve_query_seeds,
};
    ...
    S: PhoenixGraphPatchStore + PhoenixSemanticGraphPatchStore + PhoenixSemanticIndexStore,
{
    ...
    let (region_snapshot, region) = build_world_state_region(&snapshot, request, &seeds);
    let region_kernel = kernel_from_snapshot(scope, &region_snapshot)?;
    let answer = region_kernel.slot_at(phoenix_graph_kernel::KernelSlotQueryRequest {
    ...
    Ok(Some(GraphRetrievedWorldStateAnswer {
        ...
        answer: rank_world_state_answer(&region_snapshot.vertices, &answer),
        seeds,
        region,
    }))
}
