...
impl TangentCache {
    ...
    pub fn new(points: &[Vec<f32>], indices: &[usize], curvature: f32) -> Result<Self, String> {
        if points.is_empty() {
            return Err("Empty collection".to_string());
        }

        let config = PoincareConfig::with_curvature(curvature).map_err(|e| format!("{:?}", e))?;

        // Compute centroid as Fréchet mean
        let point_refs: Vec<&[f32]> = points.iter().map(|p| p.as_slice()).collect();
        let centroid = frechet_mean(&point_refs, None, &config).map_err(|e| format!("{:?}", e))?;
    ...
    pub fn recompute_centroid(&mut self, points: &[Vec<f32>]) -> Result<(), String> {
        ...
        let config =
            PoincareConfig::with_curvature(self.curvature).map_err(|e| format!("{:?}", e))?;
        let point_refs: Vec<&[f32]> = points.iter().map(|p| p.as_slice()).collect();
        self.centroid = frechet_mean(&point_refs, None, &config).map_err(|e| format!("{:?}", e))?;
